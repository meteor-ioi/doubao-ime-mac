...
本项目是一个基于 **Rust** 开发的轻量级、跨平台（Windows/macOS）语音输入工具，通过分析豆包输入法协议实现高效、实时的语音转文本功能。

## 📍 核心功能

- 🎤 **实时流式识别**：基于豆包 ASR 引擎，支持中英文混合的高精度识别。
- ⌨️ **热键唤起**：双击 `Ctrl` 键快速启动/停止语音输入（支持自定义配置）。
- 📝 **智能文本插入**：利用系统级输入模拟（Windows SendInput / macOS 辅助功能）直接插入焦点窗口。
...
| **UI 环境** | Vanilla HTML/JS, Win32 / Cocoa Native API |
| **分发打包** | GitHub Actions 自动化构建，支持双平台分发 |

## 📂 目录结构简述

- `src-tauri/src/`: 核心业务逻辑，包括 ASR 协议、音频管理及 macOS/Windows 原生适配。
